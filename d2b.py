    ...
    for row in text.split('\n'):
        ...
        for i, number in enumerate(numbers):
            for j in range(i + 1, len(numbers)):
                other = numbers[j]
                if number % other == 0:
                    debug and print('In row {} found {} and {}'.format(numbers, number, other))
                    result += number / other
                elif other % number == 0:
                    debug and print('In row {} found {} and {}'.format(numbers, number, other))
                    result += other / number
...
for expected, chars in inputs:
    print('Expected {} vs {}'.format(expected, solve(chars, expected is not None)))
